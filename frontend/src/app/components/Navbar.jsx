...
import { Menu, X, ArrowRight, Info, BookOpen, Mail, Gamepad2, Home as HomeIcon } from 'lucide-react';
...
export default function Navbar() {
  ...
  return (
    <>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className={`flex items-center justify-between transition-all duration-300 rounded-full border border-transparent ${scrolled ? 'bg-white/80 backdrop-blur-md shadow-sm border-slate-200/50 px-6 h-14' : 'bg-transparent h-16 px-2'}`}>
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={openMenu} className="md:hidden p-2 text-slate-900 rounded-full hover:bg-slate-100 transition-colors" aria-label="Open Menu">
                <Menu size={22} />
              </button>
            </div>
          </div>
      ...
      {isMobileMenuOpen && (
        <div
          className="fixed inset-0 z-[110] bg-white flex flex-col p-6 transition-all duration-250"
          style={{
        ...
        >
          <div className="flex justify-between items-center h-16 mb-8">
            ...
            <button onClick={closeMenu} className="p-2 bg-slate-50 rounded-full text-slate-900">
              <X size={24} />
            </button>
          </div>
...
function NavLink({ label, href, active }) {
  return (
    <Link href={href} className={`px-4 py-2 text-[14px] font-medium transition-all rounded-full ${active ? 'text-emerald-600' : 'text-slate-500 hover:text-slate-900 hover:bg-slate-50'}`}>
      {label}
    </Link>
  );
}
...
function MobileNavLink({ icon, label, desc, href }) {
  return (
    <Link href={href} className="flex items-center gap-4 p-4 rounded-2xl transition-all active:bg-slate-50 border border-transparent active:border-slate-100">
