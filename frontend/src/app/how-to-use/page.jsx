...
export default function HowToUse() {
  return (
    <main className="min-h-[100svh] bg-white text-slate-900 selection:bg-emerald-100 overflow-hidden">
      ...
      <div className="max-w-4xl mx-auto px-6"><div className="w-full h-[1px] bg-slate-100"></div></div>
...
function StepCard({ number, icon, title, desc, tag, delay }) {
  return (
    <div className={`anim-up ${delay} group flex flex-col md:flex-row gap-6 p-6 md:p-8 bg-white border border-slate-100 rounded-3xl hover:border-emerald-200 hover:shadow-lg hover:shadow-emerald-900/5 transition-all duration-300`}>
      <div className="flex flex-row md:flex-col items-center justify-between md:justify-start gap-4">
        <span className="text-4xl md:text-5xl font-bold text-slate-100 group-hover:text-emerald-100 transition-colors leading-none">{number}</span>
        <div className="w-14 h-14 bg-slate-50 rounded-2xl flex items-center justify-center group-hover:bg-emerald-50 transition-colors">{icon}</div>
      </div>
      <div className="flex-1 md:pt-2">
        <div className="inline-block px-3 py-1 bg-slate-50 text-[10px] font-bold text-slate-500 uppercase tracking-widest rounded-lg mb-3">{tag}</div>
